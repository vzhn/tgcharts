...
//        tw.drawSprite(tw.getTypewriter().getCornersTexture(), popupX, popupY);
        tw.drawString(date, popupX + 20, popupY, 1.0f);

        int sampleX = popupX + 20;
        int sampleY = popupY + 60;
        for (Sample s: samples) {
            tw.drawString(String.valueOf(s.value), sampleX, sampleY, 1.0f);
            tw.drawString(s.label, sampleX, sampleY + 50, 1.0f);
